...
- タグ命名: `android-vYYYY.MMDD.N`
  - 既存 container ワークフローの `vYYYY.MMDD.N` と分離。
  - `calc_tag` ジョブで `android-v` プレフィックス付きの最新番号を走査して採番。
- `pubspec.yaml` の `version` はリリース時に変更しない（手動運用のまま）。

## ジョブ構成

### 1. `calc_tag`
- `actions/github-script@v8` で `android-vYYYY.MMDD.N` の次番号を算出。
...
を使って `versionCode` を上書きする。値はタグ `android-vYYYY.MMDD.N` から下式で算出する。
...
- 例: `android-v2026.0426.1` → `26*10000000 + 4*100000 + 26*1000 + 1 = 260426001`（9桁）
...
- **`prepare` ジョブ相当は不要**: container ワークフローは複数サービスを matrix 化しているが、
  Android は単一成果物なのでフラットなジョブ構成で十分。
...
workflow_dispatch (environment: dev|prod)
        │
        ▼
   calc_tag (android-vYYYY.MMDD.N)
        │
        ▼
     build (AAB ビルド + 署名 + artifact upload)
        │
        ├──► play_upload (Internal track へ公開)
        │
