...
│   ├── migrations/               ← atlas が schema.sql の差分から自動生成（手動編集不要）
│   │   ├── 20240101000000_init.sql
│   │   └── atlas.sum             ← atlas チェックサムファイル
...
│       ├── tags.sql
│       └── users.sql
├── sqlc.yaml                     ← sqlc 設定ファイル（schema.sql を参照）
├── go.mod
├── go.sum
└── Dockerfile
```
...
スキーマは `api/db/schema.sql` に理想状態として記述します。スキーマを変更する場合は `schema.sql` を直接編集し、`atlas migrate diff` でマイグレーションファイルを生成します。
...
"go:ariga.io/atlas/cmd/atlas"          = "latest"
```
...
sleep 3
docker compose run --rm api atlas migrate apply \
  --url "$DATABASE_URL" \
